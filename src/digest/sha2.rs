...
// SHA256 functions
#[inline] fn big_s0_256(x: W32) -> W32   { rotr!(rotr!(rotr!(x, 9) ^ x, 11) ^ x, 2) }
#[inline] fn big_s1_256(x: W32) -> W32   { rotr!(rotr!(rotr!(x, 14) ^ x, 5) ^ x, 6) }
#[inline] fn small_s0_256(x: W32) -> W32 { rotr!((rotr!(x, 11) ^ x), 7)  ^ (x >> 3) }
#[inline] fn small_s1_256(x: W32) -> W32 { rotr!((rotr!(x, 2)  ^ x), 17) ^ (x >> 10) }

// SHA512 functions
#[inline] fn big_s0_512(x: W64) -> W64   { rotr!(rotr!(rotr!(x, 5) ^ x, 6) ^ x, 28) }
#[inline] fn big_s1_512(x: W64) -> W64   { rotr!(rotr!(rotr!(x, 23) ^ x, 4) ^ x, 14) }
#[inline] fn small_s0_512(x: W64) -> W64 { rotr!((rotr!(x, 7) ^ x), 1)  ^ (x >> 7) }
#[inline] fn small_s1_512(x: W64) -> W64 { rotr!((rotr!(x, 42) ^ x), 19)  ^ (x >> 6) }

pub unsafe fn sha256_block_data_order(state: &mut [u64; MAX_CHAINING_LEN / 8],
                                      data: *const u8,
                                      num: c::size_t) {
...
fn sha256_block_data_order_safe(state: &mut [u64; MAX_CHAINING_LEN / 8],
    ...
    for block in blocks {
        ...
        for t in 0..64 {
            let t1 = h + big_s1_256(e) + ch!(e, f, g) + Wrapping(K_256[t]) + w[t];
...
pub unsafe fn sha512_block_data_order(state: &mut [u64; MAX_CHAINING_LEN / 8],
                                      data: *const u8,
                                      num: c::size_t) {
...
fn sha512_block_data_order_safe(state: &mut [u64; MAX_CHAINING_LEN / 8],
    ...
    for block in blocks {
        ...
        for t in 0..80 {
            let t1 = h + big_s1_512(e) + ch!(e, f, g) + Wrapping(K_512[t]) + w[t];
