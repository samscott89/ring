...
The `native_rust` attempts to use pure Rust code for algorithm implementations.
This work is currently incomplete, and therefore some features do not yet have
native Rust implementations, in which case *ring* will fallback to using the
assembly version.
